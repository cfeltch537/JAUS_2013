...
package src.urn_jaus_jss_core_Transport_1_0;
...
import statemap.*;
import src.urn_jaus_jss_core_Transport_1_0.InternalEvents.*;
...
public void EnqueueAction(Send msg)
{
	sendJausMessage(
		msg.getBody().getSendRec().getMessagePayload().getLength(),
		msg.getBody().getSendRec().getMessagePayload().getData(),
		new JausAddress(msg.getBody().getSendRec().getDestSubsystemID(),
